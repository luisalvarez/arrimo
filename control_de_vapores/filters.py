...
'''class RawNumberForm(DateRangeFilterBaseForm):

    def __init__(self, *args, **kwargs):
        field_name = kwargs.pop('field_name')
        super(RawNumberForm, self).__init__(*args, **kwargs)
...
class RawNumberFilter(admin.filters.FieldListFilter):
    template = 'filter.html'

    def __init__(self, field, request, params, model, model_admin, field_path):
        self.lookup_kwarg = '%s' % field_path
        super(RawNumberFilter, self).__init__(
            field, request, params, model, model_admin, field_path)
        self.form = self.get_form(request)
    ...
    def get_form(self, request):
        return RawNumberForm(request, data=self.used_parameters, field_name=self.field_path)
